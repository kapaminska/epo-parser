...
- PyInstaller one-file build — push to `main` triggers GitHub Actions on `windows-latest`; download `epo-parser.zip` from run Artifacts (`epo-parser-windows`) or from [GitHub Releases](https://github.com/kapaminska/epo-parser/releases/latest) after a `v*` tag.
